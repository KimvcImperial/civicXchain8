...
async function main() {
    ...
    try {
        ...
        if (Number(nextId) > 1) {
            console.log(`\n📋 Found ${Number(nextId) - 1} commitment(s). Checking details...\n`);

            // Check each commitment
            for (let i = 1; i < Number(nextId); i++) {
                try {
                    const commitment = await contract.getCommitment(BigInt(i));
                    ...
                    console.log(`Target: ${commitment.targetValue.toString()} ${commitment.metricType}`);
                    ...
                    console.log(`Reward: ${commitment.tokenReward.toString()} tokens`);
                    console.log("");
                } catch (error) {
